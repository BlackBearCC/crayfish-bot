...
class PetClawPet {
  ...
  async init() {
    ...
    this.charSync.onAttributeChange((key, level) => {
      if (key === 'mood') {
        ...
        } else if (level === 'subhealthy') {
          this.bubble.show('身体有点不对劲...', 3000);
    ...
    // 属性数值变化 → 浮动 +/- 文字
    this.floatText = new FloatText(document.getElementById('pet-area'));
    this.charSync.onValueDelta((key, delta) => {
      this.floatText.show(key, delta);
    });
  ...
  _handleChatEval({ intent } = {}) {
    const BUBBLES = {
      praise:    ['嘿嘿，主人夸我了~', '被夸了！好开心！', '喵喵喵！今天好棒！'],
      playful:   ['嘻嘻嘻～', '哈哈哈好好玩！'],
    ...
    const msgs = BUBBLES[intent];
    if (msgs) this.bubble.show(msgs[Math.floor(Math.random() * msgs.length)], 3000);
  }
