...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.PETCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".petclaw", `workspace-${profile}`);
  }
  return path.join(home, ".petclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".petclaw";
